...
contract Spotter is LibNote {
    ...
    function poke_to_bind(address osm_addr) external {
        updatePrice.bind(osm_addr, "PriceFeedUpdate(bytes32)");
    }

    function poke_to_detach(address osm_addr) external {
        updatePrice.detach(osm_addr, "PriceFeedUpdate(bytes32)");
    }
    ...
    constructor() public {
        ...
        par = ONE;
        live = 1;
        handler.create_handler("update(bytes32)", 1000000, 120);
    }
